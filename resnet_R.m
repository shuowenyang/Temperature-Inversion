...
net = replaceLayer(net, 'input_1',newLayers(1));
net = replaceLayer(net, 'fc1000', newLayers(2));

net = replaceLayer(net, 'fc1000_softmax', newLayers(3));
...
%% 5.指定训练选项
   options = trainingOptions('sgdm', ...%求解器，'sgdm'（默认） | 'rmsprop' | 'adam'
    ...
    'ValidationFrequency',5, ...%验证频率，即每间隔多少次迭代进行一次验证
    'MiniBatchSize',256, ...
    'LearnRateSchedule','piecewise', ...%是否在一定迭代次数后学习速率下降
    'LearnRateDropFactor',0.5, ...%学习速率下降因子
