...
% net_rbf = newrb(p_train, t_train, 7*10^-4,100);
...
disp(['RBF训练集数据的RMSE为：', num2str(error1)])
...
disp(['RBF训练集数据的MdAPE为：', num2str(mdape1)])
disp(['RBF测试集数据的MdAPE为：', num2str(mdape2)])
% MBE
...
% save A_RBF T_sim1 T_train T_sim2 T_test error_BP
